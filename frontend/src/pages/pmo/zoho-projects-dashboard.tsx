export default function ZohoProjectsDashboard({ onBack }: { onBack: () => void }) {
  const cacheBust = Math.floor(Date.now() / 20000);
  return (
    <div style={{ display: 'flex', flexDirection: 'column', height: 'calc(100vh - 120px)' }}>
      <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 12 }}>
        ...
        <span style={{ color: 'var(--ccm-gray-dark)', fontSize: 12, fontWeight: 700, textTransform: 'uppercase', letterSpacing: '.1em' }}>Dashboard Zoho Projects</span>
      </div>
      <iframe
        src={`https://war.ccmcloud.com.br/ccm/warriors_projects.html?v=${cacheBust}`}
        style={{ flex: 1, width: '100%', border: 'none', borderRadius: 8, boxShadow: '0 2px 12px rgba(12,25,33,.1)' }}
        title="Dashboard Zoho Projects"
