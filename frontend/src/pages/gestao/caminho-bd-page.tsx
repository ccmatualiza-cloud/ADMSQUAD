...
export default function CaminhoBdPage({ onBack }: { onBack: () => void }) {
  ...
  const fetchStatus = async () => {
    setLoading(true);
    try {
      const d = await http.get<{ caminhobd: string; databd: string }>('/api/gestao/caminho-bd/status');
      setStatus((d.caminhobd === 'S' ? 'S' : 'N') as 'S' | 'N');
      setDatabd(d.databd || '');
  ...
  const handleToggle = async () => {
    ...
    setSaving(true);
    try {
      const r = await http.put<{ updated: boolean; caminhobd: string; databd: string }>(
        '/api/gestao/caminho-bd/update', { valor: novoValor }
      );
      setStatus(r.caminhobd === 'S' ? 'S' : 'N');
      setDatabd(r.databd || '');
      toast.success(novoValor === 'S' ? 'Caminho BD habilitado!' : 'Caminho BD desabilitado!');
  ...
  return (
    <div>
      ...
      <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 4 }}>
        ...
        <span style={{ color: 'var(--ccm-gray-dark)', fontSize: 12, fontWeight: 700, textTransform: 'uppercase', letterSpacing: '.1em' }}>Caminho BD</span>
      </div>
      ...
      <div style={{ maxWidth: 480, margin: '40px auto' }}>
        <div style={{ background: '#fff', border: '1px solid var(--ccm-line)', borderRadius: 10, padding: '36px 40px', boxShadow: '0 2px 12px rgba(12,25,33,.08)', textAlign: 'center' }}>
          ...
          <div style={{ fontSize: 13, fontWeight: 700, textTransform: 'uppercase', letterSpacing: '.14em', color: 'var(--ccm-gray-dark)', marginBottom: 32 }}>
            Alterar Caminho do Banco de Dados dos Atualizadores
          </div>
