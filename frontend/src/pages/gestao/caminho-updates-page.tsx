...
export default function CaminhoUpdatesPage({ onBack }: { onBack: () => void }) {
  const [status, setStatus]   = useState<'S' | 'N' | null>(null);
  const [dataup, setDatabd]   = useState('');
  ...
  const fetchStatus = async () => {
    setLoading(true);
    try {
      const d = await http.get<{ caminhoup: string; dataup: string }>('/api/gestao/caminho-updates/status');
      setStatus((d.caminhoup === 'S' ? 'S' : 'N') as 'S' | 'N');
      setDatabd(d.dataup || '');
  ...
  const handleToggle = async () => {
    ...
    setSaving(true);
    try {
      const r = await http.put<{ updated: boolean; caminhoup: string; dataup: string }>(
        '/api/gestao/caminho-updates/update', { valor: novoValor }
      );
      setStatus(r.caminhoup === 'S' ? 'S' : 'N');
      setDatabd(r.dataup || '');
      toast.success(novoValor === 'S' ? 'Caminho Updates habilitado!' : 'Caminho Updates desabilitado!');
  ...
  return (
    <div>
      ...
      <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 4 }}>
        ...
        <span style={{ color: 'var(--ccm-gray-dark)', fontSize: 12, fontWeight: 700, textTransform: 'uppercase', letterSpacing: '.1em' }}>Caminho Updates</span>
      </div>
